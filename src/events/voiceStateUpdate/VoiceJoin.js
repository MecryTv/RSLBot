...
const DateTimeService = require("../../services/DateTimeService"); // Service importieren
        ...
        if (!oldState.channelId && newState.channelId) {
            ...
            if (logChannel) {
                ...
                if (targetChannel) {
                    ...
                    const title = `🔊 Voice Session Connected`;

                    const description =
                        `👤 **User:** ${user} (${user.tag})\n` +
                        `🆔 **Account ID:** \`${user.id}\`\n` +
                        `📍 **Channel:** ${newState.channel}\n` +
                        `🔌 **Session ID:** \`${newState.channelId}\`\n` +
                        `📅 **Date:** ${date}\n` +
                        `🕒 **Time:** ${time} (CET)`;
                    ...
                    await targetChannel.send({
                        ...
                        flags: 32768
                    }).catch(err => logger.error(`[VoiceJoin] Failed to send container components: ${err.message}`));
                }
            }
        }
    }
